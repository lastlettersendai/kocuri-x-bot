...
def job():
    ...
    # --- 重要：Railwayの「Variables（変数）」から安全に読み込む ---
    # これにより、GitHub上に生のAPIキーが表示されなくなります
...
def job():
    ...
    try:
        ...
        # 404エラーを防ぐため、api_version='v1' を指定して接続します
...
# 1. 起動時にまず1回テスト投稿を実行する
job()

# 2. 毎日 09:30 に定期投稿する設定（お好みの時間に変更可能です）
schedule.every().day.at("09:30").do(job)
